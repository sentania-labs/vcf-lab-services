...
quarantine. The Compose test enforces the latest-tracking published-image defaults with
their override variables intact,
...
Kubernetes manifest test also asserts the product images default to the
latest tags with the pin-in-deployment note at every site.
