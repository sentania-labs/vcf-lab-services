...
# The source checkout builds the product, so its defaults track latest and the
# override variables stay in place for deployments and release bundles to pin.
[ "$(grep -c 'image: ${VCF_SERVICES_UI_IMAGE:-ghcr.io/sentania-labs/vcf-lab-services/ui:latest}' docker-compose.yml)" -eq 2 ] \
	|| fail "bootstrap and UI do not default to the latest published image through VCF_SERVICES_UI_IMAGE"
grep -q 'image: ${VCF_SERVICES_SYNC_IMAGE:-ghcr.io/sentania-labs/vcf-lab-services/sync-base:latest}' docker-compose.yml \
	|| fail "sync does not default to the latest published image through VCF_SERVICES_SYNC_IMAGE"
grep -q 'image: ${VCF_SERVICES_SFTP_IMAGE:-ghcr.io/sentania-labs/vcf-lab-services/sftp:latest}' docker-compose.yml \
	|| fail "SFTP does not default to the latest published image through VCF_SERVICES_SFTP_IMAGE"
! grep -Eq 'vcf-lab-services/(ui|sync-base|sftp):v[0-9]' docker-compose.yml \
	|| fail "a Compose default still pins a concrete release tag; pinning belongs in deployments"
[ "$(grep -c 'Deployments must pin an exact release tag' docker-compose.yml)" -eq 4 ] \
	|| fail "every Compose image default needs the pin-in-deployment note"
pinned_render="$(VCF_SERVICES_UI_IMAGE=example/ui:v9.9.9 VCF_SERVICES_SYNC_IMAGE=example/sync:v9.9.9 \
	VCF_SERVICES_SFTP_IMAGE=example/sftp:v9.9.9 docker compose config)"
